...
resource "aws_ssm_parameter" "govnotify_invite_no_organisation_template_id" {
  type        = "SecureString"
  key_id      = aws_kms_key.local_transcribe_secrets.arn
  name        = "/local-transcribe/govnotify/invite_no_organisation_template_id"
  description = "GovNotify invite no organisation email template ID"
  value       = "placeholder" # Update value in SSM - Do not hardcode
