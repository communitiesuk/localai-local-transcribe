resource "aws_kms_key" "main" {
  ...
  description         = "minute-cloudtrail-${var.environment_name}"
}
...
data "aws_iam_policy_document" "cloudtrail_kms" {
  ...
  statement {
    ...
    condition {
      ...
      values   = ["arn:aws:cloudtrail:${data.aws_region.current.region}:${data.aws_caller_identity.current.account_id}:trail/minute-cloudtrail-${var.environment_name}"]
    }
  }
}