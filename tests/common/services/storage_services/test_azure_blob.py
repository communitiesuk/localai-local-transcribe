...
@pytest.mark.asyncio
async def test_upload(mocker, mock_client_ctx, mock_container_client,mock_file_ctx):
    ...
    await AzureBlobStorageService.upload(key, Path("/tmp/file.mp3"))
...
@pytest.mark.asyncio
async def test_download(mocker, mock_client_ctx, mock_container_client, mock_file_ctx):

    blob_key = "/tmp/blob.mp3"
    ...
    await AzureBlobStorageService.download(blob_key, Path("/tmp/file.mp3"))
...
async def test_generate_presigned_url_put_object(mocker, mock_client_ctx, mock_container_client):
    ...
    sas_token = "local_transcribe_token"
    ...
    mock_generate_sas = mocker.patch("common.services.storage_services.azure_blob.generate_blob_sas", return_value=sas_token)


    result = await AzureBlobStorageService.generate_presigned_url_put_object(object_key, expiry_seconds=expiry_period)

    assert result == f"{mock_container_client.url}/{object_key}?{sas_token}"
...
async def test_generate_presigned_url_get_object(mocker, mock_client_ctx, mock_container_client):
    ...
    assert result == f"{mock_container_client.url}?{sas_token}"
    mock_generate_sas.assert_called_once()
