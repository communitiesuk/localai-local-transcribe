...
async def test_record_analytics_event_skips_when_no_evaluation_id(async_session, evaluation_id):
    await record_analytics_event(async_session, AnalyticsEventType.USER_INVITED, evaluation_id)
...
async def test_record_analytics_event_swallows_and_rolls_back_on_failure(async_session):
    ...
    # must not raise
    await record_analytics_event(async_session, AnalyticsEventType.USER_INVITED, "EVAL-001")
