...
def mock_user():
    user = Mock()
    user.id = uuid.uuid4()
    user.email = "test@transcribe.com"
    return user
...
@pytest.mark.asyncio
async def test_get_user_templates_success(mock_session, mock_user):
    tpl = Mock()
    tpl.id = uuid.uuid4()
    ...
    res = await get_user_templates(mock_user, mock_session)
    assert len(res) == 1
...
@pytest.mark.asyncio
async def test_get_user_template_success_and_not_found(mock_session, mock_user):
    tpl = Mock()
    tpl.id = uuid.uuid4()
    tpl.updated_datetime = datetime.now(tz=UTC)
    tpl.name = "Test Template"
    ...
    res = await get_user_template(mock_user, mock_session, tpl.id)
    assert res.id == tpl.id

    exec_result.first.return_value = None
    mock_session.exec.return_value = exec_result
    with pytest.raises(HTTPException):
        await get_user_template(mock_user, mock_session, tpl.id)


@pytest.mark.asyncio
async def test_create_user_template_calls_add_and_commit(mock_session, mock_user):
...
@pytest.mark.asyncio
async def test_edit_user_template_updates_and_questions_branch(mock_session, mock_user):
    tpl = Mock()
    tpl.id = uuid.uuid4()
    tpl.user_id = mock_user.id
    tpl.updated_datetime = datetime.now(tz=UTC)
    exec_result = Mock()
    exec_result.first.return_value = tpl
    mock_session.exec.return_value = exec_result
    ...
    await edit_user_template(mock_user, mock_session, tpl.id, request)
    mock_session.add.assert_called()
    mock_session.commit.assert_awaited()
...
@pytest.mark.asyncio
async def test_delete_user_template_success_and_not_found(mock_session, mock_user):
    tpl = Mock()
    tpl.id = uuid.uuid4()
    tpl.user_id = mock_user.id
    exec_result = Mock()
    ...
    await delete_user_template(mock_user, mock_session, tpl.id)
...
@pytest.mark.asyncio
async def test_duplicate_user_template_success_and_not_found(mock_session, mock_user):
    original = Mock()
    original.id = uuid.uuid4()
    original.user_id = mock_user.id
    ...
    await duplicate_user_template(mock_user, mock_session, original.id)
    mock_session.add.assert_called()
    mock_session.commit.assert_awaited()
