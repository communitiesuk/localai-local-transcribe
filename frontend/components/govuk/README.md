...
**Worked example:** [`app/settings/page.tsx`](../../app/settings/page.tsx) ΓÇö data-retention form using fieldset, radios, and button wrappers with React Hook Form.
...
| Spacing / one-off layout tweaks                       | Tailwind utilities on a wrapper `div` only ΓÇö do not restyle govuk components           |
| Dialog, popover, tooltip, toast, rich-text menus      | Radix primitives under `@/components/ui/*` ΓÇö see [Radix exceptions](#radix-exceptions) |
...
Everything else (button, input, label, radio, checkbox, select, tabs, card, badge, alert, separator) has a GOV.UK Design System equivalent ΓÇö use or add a `govuk/` wrapper instead.
...
`GovukErrorSummary` accepts either `errorList={[{ href: '#field', text: 'ΓÇª' }]}` or `errors={form.formState.errors}` (uses each field's `message`).

## Adding a new wrapper

1. Find the [GOV.UK Design System HTML reference](https://design-system.service.gov.uk/components/) for the component.
2. Copy the nearest existing wrapper (`button.tsx`, `radios.tsx`, ΓÇª) ΓÇö keep markup canonical; use `cn()` only for optional caller `className`.
