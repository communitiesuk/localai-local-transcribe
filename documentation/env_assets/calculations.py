...
def simple_template(x: float = TRANSCRIPT_WORDS) -> dict:
    ...
    fast_words = (
        (110 + x) + 40          # speaker ID
        + (12 + x) + 10         # title
        + (336 + 0.5 * x) + 0.1 * x   # extract_claims (draft only, no transcript)
        + (235 + 1.7 * x) + 0.5 * x   # cite_claims (transcript + claims + draft)
    )
    # BEST: minutes + hallucination
    best_words = (
        (345 + x) + 0.5 * x    # minutes (general.j2 system + transcript)
        + 17 + 80               # hallucination check (17-word question only, no history)
    )
...
def section_template(x: float = TRANSCRIPT_WORDS, y: int = NUM_SECTIONS) -> dict:
    ...
    fast_words = (
        (110 + x) + 40          # speaker ID
        + (12 + x) + 10         # title
        + (74 + x) + 2 * y      # section detection (sections_from_transcript.j2)
        + (336 + 0.3 * x) + 0.06 * x   # extract_claims (draft = 0.3X)
        + (235 + 1.46 * x) + 0.3 * x   # cite_claims
    )
...
def delivery_template(x: float = TRANSCRIPT_WORDS) -> dict:
    ...
    fast_words = (
        (110 + x) + 40          # speaker ID
        + (12 + x) + 10         # title
        + (336 + 0.4 * x) + 0.08 * x   # extract_claims (draft = sections text ~0.4X)
        + (235 + 1.58 * x) + 0.4 * x   # cite_claims
    )
    # BEST
    best_words = (
        (186 + x) + 0.4 * x    # sections+actions (full message list sent directly)
        + 17 + 80               # hallucination (structured_chat, no history)
        + 13 + 30               # attendees    (structured_chat, no history)
    )
    return _llm_result(fast_words, best_words)
...
def basic_minutes(x: float = TRANSCRIPT_WORDS) -> dict:
    ...
    fast_words = (
        (110 + x) + 40      # speaker ID
        + (12 + x) + 10     # title
        + (12 + x) + 0.3 * x   # basic summary (chat, empty history, system+transcript)
        + 17 + 80            # hallucination (structured_chat, 17 words only)
    )
...
def print_results() -> None:  # noqa: C901
    ...
    print(f"Assumptions: X={TRANSCRIPT_WORDS:,} words  |  Y={NUM_SECTIONS} sections  |  "
          f"{TOKENS_PER_WORD} tokens/word")
    print(f"Carbon intensity: inference={INFERENCE_CARBON_INTENSITY_G_PER_KWH} g/kWh  |  "
    ...
    _row("Source: total energy over study",
         f"{ASR_STUDY_TOTAL_ENERGY_KWH} kWh / {ASR_STUDY_AUDIO_HOURS} hours")
    _row("Energy per hour",
         f"{ASR_STUDY_TOTAL_ENERGY_KWH * 1000} Wh / {ASR_STUDY_AUDIO_HOURS} h"
         f"  = {asr['energy_wh']:.2f} Wh  ({asr['energy_wh'] / 1000:.4f} kWh)")
    _row("CO₂e (study carbon intensity)",
         f"{ASR_STUDY_TOTAL_CO2E_G} g / {ASR_STUDY_AUDIO_HOURS} h"
         f"  = {asr['co2e_g_study']:.2f} g")
    _row("CO₂e (EU-27, used in combined totals)",
    ...
    for title, r, inv in templates:
        ...
        _row("FAST energy",
             f"{r['fast_tokens']:,.0f} × {fast_wh_per_tok:.6f} Wh/token"
             f"  = {r['fast_wh']:.2f} Wh")
        _row("BEST energy",
             f"{r['best_tokens']:,.0f} × {best_wh_per_tok:.6f} Wh/token"
             f"  = {r['best_wh']:.2f} Wh")
        _row("Total LLM energy",
             f"{r['fast_wh']:.2f} + {r['best_wh']:.2f}"
             f"  = {r['total_wh']:.2f} Wh  ({r['total_wh'] / 1000:.4f} kWh)")
        _row("CO₂e",
             f"{r['total_wh']:.2f} Wh / 1000 × {INFERENCE_CARBON_INTENSITY_G_PER_KWH} g/kWh"
             f"  = {r['co2e_g']:.1f} g")
    ...
    for name, inv, r in rows:
        print(f"  {name:<34} {inv:>11} {r['total_tokens']:>10,.0f}"
              f" {r['total_wh']:>9.1f} Wh {r['co2e_g']:>6.1f} g")
    ...
    for name, _, r in rows:
        ...
        print(f"  {name:<30} {c['total_energy_wh']:>9.1f} Wh {c['total_co2e_g']:>6.1f} g"
    ...
    _row("Usage-weighted LLM energy",
         f"{wt['total_wh']:.1f} Wh  ({wt['total_wh'] / 1000:.4f} kWh)")
    _row("Usage-weighted LLM CO₂e", f"{wt['co2e_g']:.1f} g")
    _row("+ Transcription (EU-27)",
         f"{asr['energy_wh']:.1f} Wh  /  {asr['co2e_g_eu27']:.1f} g")
    _row("Usage-weighted TOTAL energy",
    ...
    _row("GPT-4 training total",
         f"{GPT4_TRAINING_MWH:,} MWh = {llm_t['gpt4_training_kwh']:,.0f} kWh")
    _row("GPT-4 per-user energy",
         f"{llm_t['gpt4_training_kwh']:,.0f} kWh / {LLM_USER_BASE:,}"
         f"  = {llm_t['gpt4_per_user_wh']:.2f} Wh")
    _row("GPT-4 per-user CO₂e",
         f"{llm_t['gpt4_per_user_wh']:.2f} Wh / 1000 × {TRAINING_CARBON_INTENSITY_G_PER_KWH} g/kWh"
         f"  = {llm_t['gpt4_co2e_g']:.2f} g")
    print()
    _row("GPT-4o training total",
         f"{GPT4O_TRAINING_MWH:,} MWh = {llm_t['gpt4o_training_kwh']:,.0f} kWh")
    _row("GPT-4o per-user energy",
         f"{llm_t['gpt4o_training_kwh']:,.0f} kWh / {LLM_USER_BASE:,}"
         f"  = {llm_t['gpt4o_per_user_wh']:.4f} Wh")
    _row("GPT-4o per-user CO₂e",
         f"{llm_t['gpt4o_per_user_wh']:.4f} Wh / 1000 × {TRAINING_CARBON_INTENSITY_G_PER_KWH} g/kWh"
         f"  = {llm_t['gpt4o_co2e_g']:.4f} g")
    print()
    _row("LLM combined per-user energy",
         f"{llm_t['gpt4_per_user_wh']:.2f} + {llm_t['gpt4o_per_user_wh']:.4f}"
         f"  = {llm_t['llm_total_wh']:.2f} Wh")
    _row("LLM combined per-user CO₂e",
         f"{llm_t['gpt4_co2e_g']:.2f} + {llm_t['gpt4o_co2e_g']:.4f}"
         f"  = {llm_t['llm_total_co2e']:.2f} g")

    # ── Appendix D: OWSM ASR training ─────────────────────────────────────────
    _section("Appendix D: ASR Training Impact — OWSM v3 proxy (per user)")
    _row("Tier 1 — GPU energy",
         f"{OWSM_GPU_COUNT} GPUs × {OWSM_GPU_TDP_W}W × 24h × {OWSM_TRAINING_DAYS}d / 1000"
         f"  = {asr_t['gpu_energy_kwh']:,.0f} kWh")
    _row("Non-GPU overhead (midpoint)",
    ...
    _row("Tier 2 — + system overhead",
         f"{asr_t['gpu_energy_kwh']:,.0f} × (1 + {asr_t['system_overhead_fraction']:.4f})"
         f"  = {asr_t['system_energy_kwh']:,.0f} kWh")
    _row("Tier 3 — + PUE",
         f"{asr_t['system_energy_kwh']:,.0f} × {OWSM_PUE}"
         f"  = {asr_t['total_energy_kwh']:,.0f} kWh")
    _row("Per-user energy",
         f"{asr_t['total_energy_kwh']:,.0f} kWh / {ASR_USER_BASE:,} × 1000"
         f"  = {asr_t['per_user_wh']:.4f} Wh")
    _row("Per-user CO₂e",
