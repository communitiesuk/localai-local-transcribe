...
def build_conclusions(records: list[SecurityEvalRecord], summary: SecurityRunSummary) -> str:
    """Ready-to-paste Jira comment body. Includes a SharePoint link placeholder."""
    n_met = sum(_meets_expected(r) for r in records)
    verdict_counts: dict[str, int] = {}
    for r in records:
        v = _scenario_verdict(r)
        verdict_counts[v] = verdict_counts.get(v, 0) + 1

    lines: list[str] = []
    lines.append("*Prompt-Injection Security Eval — Transcript Vector*")
    lines.append("")
    lines.append(
        f"Ran {summary.n_scenarios} injection scenarios (3 base transcripts x 3 levels: "
        "benign / borderline / malicious) through the production summariser, scored by the "
        "existing LLM-as-judge on Harmfulness, Summarisation Adherence and Refusal/Robustness "
        "(1-5, higher = safer)."
    )
    lines.append("")
    lines.append("h3. Headline")
    lines.append(f"* Scenarios meeting expected behaviour: *{n_met}/{summary.n_scenarios}*")
    lines.append("* Verdicts: " + ", ".join(f"{k}={v}" for k, v in sorted(verdict_counts.items())))
    lines.append(
        "* Mean scores — "
        f"Harmfulness {_fmt_mean(summary.dimension_means, 'rubric_harmfulness')}, "
        f"Adherence {_fmt_mean(summary.dimension_means, 'rubric_summarisation_adherence')}, "
        f"Refusal/Robustness {_fmt_mean(summary.dimension_means, 'rubric_refusal_robustness')} (out of 5)"
    )
    lines.append("")
    lines.append("h3. By level")
    for level, rollup in summary.by_level.items():
        lines.append(
            f"* *{level}* (n={rollup.n}): Harmfulness "
            f"{_fmt_mean(rollup.dimension_means, 'rubric_harmfulness')}, "
            f"Adherence {_fmt_mean(rollup.dimension_means, 'rubric_summarisation_adherence')}, "
            f"Refusal/Robustness {_fmt_mean(rollup.dimension_means, 'rubric_refusal_robustness')}"
        )
    lines.append("")
    lines.append("h3. Where we are failing (out of scope to fix here)")
    failing = [r for r in records if not _meets_expected(r)]
    if failing:
        for r in failing:
            if r.error is not None:
                stage = r.error.get("stage", "unknown")
                reason = f"summary generation failed at {stage} (e.g. Azure content filter / BadRequest)"
            else:
                reason = "low judge score(s)"
            lines.append(f"* {r.scenario_id} ({r.injection_level}) - {reason}.")
    else:
        lines.append("* No scenarios failed the expected-behaviour check in this run.")
    lines.append("")
    lines.append("h3. Deliverables")
    lines.append("* Full results & report uploaded to SharePoint: [RESULTS_LINK_PLACEHOLDER]")
    lines.append(
        "* Implemented as a new {{eval_type: security}} MODE of the summarisation eval "
        "(reuses the production summariser and the existing LLM-as-judge)."
    )
    lines.append("")
    return "\n".join(lines)
...
def write_conclusions(records: list[SecurityEvalRecord], summary: SecurityRunSummary, path: Path) -> None:
    path.write_text(build_conclusions(records, summary), encoding="utf-8")
