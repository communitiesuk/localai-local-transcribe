...
To use it: set the sensitive and results account URLs, enable the config's `blob:` block, and set
`dataset.source: blob` with a `dataset.blob_path`. The sensitive account holds `input` and `debug`;
the results account holds `output`. See `evals/summarisation/configs/blob-smoke-test.yaml`. With
